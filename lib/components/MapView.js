...
const propTypes = {
  ...
  clusterItemIcons: PropTypes.array,
  clusterColors: PropTypes.array,
  clusterBuckets: PropTypes.array,
...
class MapView extends React.Component {
  ...
  addClusterItems(arr) {
    this._runCommand('addClusterMarkers', [arr]);
  }

  removeClusterItem(id) {
    this._runCommand('removeClusterMarker', [id]);
  }

  removeClusterItems(ids) {
    this._runCommand('removeClusterMarkers', [ids]);
  }

  removeAllClusterItem() {
    this._runCommand('removeAllClusterMarker', []);
  }
  ...
  render() {
    ...
    if (this.state.isReady) {
      props = {
        ...
        onClusterItemClick: this._onClusterItemClick,
        ...this.props,
      };
      if (
        Platform.OS === 'ios' &&
    ...
    const {clusterColors, ...p} = props;
    let processedColors = null;
    if (clusterColors) {
      if (Platform.OS === 'android') {
        processedColors = clusterColors;
      } else {
        processedColors = clusterColors.map(processColor);
      }
    }
    return (
      <AIRMap
